...
import {
  mockEngagements, mockArtifacts, mockTasks, mockDecisions,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4 pb-20 md:pb-4">
        ...
        {mockEngagements.map((engagement) => (
          <EngagementDetail key={engagement.id} engagement={engagement} />
        ))}
      </div>
    </div>
  );
...
function EngagementDetail({ engagement }: { engagement: Engagement }) {
  const artifacts = mockArtifacts.filter((a) => a.engagementId === engagement.id);
  const tasks = mockTasks.filter((t) => t.engagementId === engagement.id);
  const decisions = mockDecisions.filter((d) => d.engagementId === engagement.id);
